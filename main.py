...
# Stripe Price IDs Ч set in Railway env vars after creating products in Stripe Dashboard
...
# Minimum client version allowed to validate (Phase 7 Ч version locking)
...
async def _send_license_email(email: str, key: str, plan: str, expires_at) -> bool:
    if not RESEND_API_KEY:
        log.warning("RESEND_API_KEY not set Ч skipping email to %s", email)
        return False
...
<div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#1a1a2e">
  ...
  <p style="margin-top:24px;color:#9ca3af;font-size:12px">
    Keep this email Ч your key is the only way to manage your license.<br>
...
_MANIFEST = {
    ...
    "notes": "‘икс Stories/–еакций: воркфлоу-узлы теперь реально просматривают и реагируют на истории. ‘икс рассылки (не уходит в »збранное). јвто-сохранение видео в выбранную папку. Ёкспорт аккаунтов (Session/JSON/CSV). ETA-прогресс в узлах воркфлоу.",
}
